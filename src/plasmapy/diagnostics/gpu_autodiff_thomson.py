...
def chi(
    ...
    outer_frac = torch.tensor(1.-inner_frac, device=device)

    m_inner = torch.linspace(0, inner_range, int(torch.floor(torch.tensor([nPoints / 2 * inner_frac]))), device=device)
    p_inner = torch.linspace(0, inner_range, int(torch.ceil(torch.tensor([nPoints / 2 * inner_frac]))), device=device)
    m_outer = torch.linspace(inner_range, 1, int(torch.floor(torch.tensor([nPoints / 2 * outer_frac]))), device=device)
    p_outer = torch.linspace(inner_range, 1, int(torch.ceil(torch.tensor([nPoints / 2 * outer_frac]))), device=device)
    ...
    zm = torch.zeros((len(xi), len(m)), device=device)
    zp = torch.zeros((len(xi), len(p)), device=device)
    ...
    m_point_array = torch.tensor(phi + m * deltauMax, device=device)
    p_point_array = torch.tensor(phi + p * deltauMax, device=device)

    m_deltas = torch.concatenate((torch.tensor(torch.tensor(m_point_array[1:], device=device)
                                                - torch.tensor(m_point_array[:-1], device=device)),
                               torch.tensor([0.], device=device)))

    p_deltas = torch.concatenate((torch.tensor(torch.tensor(p_point_array[1:], device=device) 
                                               - torch.tensor(p_point_array[:-1], device=device)),
                               torch.tensor([0.], device=device)))
    ...
    m_SI = torch.tensor([particle_m * 1.6605e-27], device=device)
    q_SI = torch.tensor([particle_q * 1.6022e-19], device=device)
...
def fast_spectral_density_arbdist(
    ...
    electron_vel = torch.tensor([], device=device)  # drift velocities (vector)
    electron_vel_1d = torch.tensor([], device=device) # 1D drift velocities (scalar)
    vTe = torch.tensor([], device=device)  # thermal speeds (scalar)
    ...
    for i, fn in enumerate(efn):
        ...
        electron_vel_1d = torch.concatenate((electron_vel_1d, torch.tensor([bulk_velocity], device=device)))
        vTe = torch.concatenate((vTe, torch.tensor([torch.sqrt(torch.trapz(moment2_integrand, v_axis))], device=device)))

    electron_vel = torch.reshape(electron_vel, (len(efn), 3))

    ion_vel = torch.tensor([], device=device)
    ion_vel_1d = torch.tensor([], device=device)
    vTi = torch.tensor([], device=device)
    ...
    for i, fn in enumerate(ifn):
        ...
        ion_vel_1d = torch.concatenate((ion_vel_1d, torch.tensor([bulk_velocity], device=device)))
        vTi = torch.concatenate((vTi, torch.tensor([torch.sqrt(torch.trapz(moment2_integrand, v_axis))], device=device)))

    ion_vel = torch.reshape(ion_vel, (len(ifn), 3))

    # Define some constants
    C = torch.tensor([299792458], dtype = torch.float64, device=device)  # speed of light

    # Calculate plasma parameters
    #zbar = torch.sum(ifract * ion_z)    # UNCOMMENT LINE 356
    zbar = torch.sum(torch.tensor(ifract, device=device) * ion_z)
    ne = efract * n
    ni = torch.tensor(ifract, device=device) * n / zbar  # ne/zbar = sum(ni)
    ...
    n = torch.tensor(n * 3182.60735, dtype = torch.float64, device=device)
    wpe = torch.sqrt(n)

    # Convert wavelengths to angular frequencies (electromagnetic waves, so
    # phase speed is c)
    ws = torch.tensor(2 * torch.pi * C / wavelengths, device=device)
    wl = torch.tensor(2 * torch.pi * C / probe_wavelength, device=device)
    ...
    chiE = torch.zeros((len(efract), len(w)), dtype=torch.complex128, device=device)
    ...
    chiI = torch.zeros((len(ifract), len(w)), dtype=torch.complex128, device=device)
    for i in range(len(ifract)):
        chiI[i, :] = chi(
            f=ifn[i],
            derivative_matrices=derivative_matrices,
            u_axis=(i_velocity_axes[i] - ion_vel_1d[i])
            / (torch.sqrt(torch.tensor([2], device=device)) * vTi[i]),
            k=k,
            xi=xii[i],
            v_th=vTi[i],
            n=ni[i],
    ...
    econtr = torch.zeros((len(efract), len(w)), dtype=torch.complex128, device=device)
    for m in range(len(efract)):
        econtr[m] = efract[m] * (
            2
            * torch.pi
    ...
    # ion component
    icontr = torch.zeros((len(ifract), len(w)), dtype=torch.complex128, device=device)
    for m in range(len(ifract)):
        icontr[m] = ifract[m] * (
            2
            * torch.pi
...
def spectral_density_arbdist(
    ...
    if efract is None:
        efract = torch.ones(1, device=device)
    else:
        efract = torch.tensor(efract, dtype=torch.float64, device=device)

    if ifract is None:
        ifract = torch.ones(1, device=device)
        
    #Check for notches
    if notches is None:
        notches = torch.tensor([[520, 540]], device=device) # * u.nm
    ...
    ion_z = torch.zeros(len(ion_species), device=device)
    ion_m = torch.zeros(len(ion_species), device=device)
